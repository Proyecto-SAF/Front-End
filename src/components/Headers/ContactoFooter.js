...
let ContactoShema = yup.object().shape({
...
function ContactoFooter() {
  ...
  useEffect(()=> {
    ContactoShema.isValid({nombreCompleto, correo, documento, mensaje})
    .then(
      (valid) => {
        if(valid){
          setHabilitado(true)
  ...
  },[nombreCompleto, correo, documento, mensaje,  ContactoShema]);

  const history = useHistory()
  
  const registroContacto = async () => {
